-- Reworks the level system twice over:
--   1. The overall level is now the 0-100 score directly (clamped to a
--      minimum of 1) instead of a coarse 1-4 tier — "level 100 is max."
--   2. Each individual skill now gets its own 1-100 level (the same
--      shipping/activity/engagement/depth shape as the overall formula,
--      just scoped to only the projects that use that particular tech)
--      instead of a bare project count — the profile page shows this
--      instead of ever asking someone to type their skills in by hand.
...
begin
  ...
  computed_score := greatest(1, round(
    shipping_score * 0.30 + tech_score * 0.15 + activity_score * 0.20
    + engagement_score * 0.20 + portfolio_score * 0.15
  ));

  computed_label := case
    when computed_score >= 76 then 'Expert'
    when computed_score >= 51 then 'Advanced'
    when computed_score >= 26 then 'Intermediate'
    else 'Beginner'
  end;

  -- Per-skill levels: same shape as the overall formula (shipping,
  -- activity, engagement, a "depth" stand-in for portfolio size) but each
  -- signal is computed only from the subset of projects using that one
  -- tech, so two people who both know Rust don't get the same number
  -- just because they're both "using Rust" — someone who shipped 3 real
  -- Rust projects outranks someone with one idea-stage repo.
  ...
  values (
    target_profile_id, computed_score, computed_label, computed_score, skills_json, proj_count,
    format(
