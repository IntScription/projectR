-- Split into its own migration from 20260818000000 deliberately: Postgres
-- won't let a transaction both add an enum value and reference that same
-- value, so the 'level_up' addition has to commit first before this file
-- (which uses it) can run.
--
-- `refresh_profile_level` gets exactly two additions on top of its
-- existing body (unchanged scoring formula): capture the profile's level
-- *before* the upsert, and if the new level crossed into a new 100-tier
-- (100, 200, ... 10,000 — the same boundaries `Achievement.swift` already
-- unlocks on), insert a `level_up` notification. Catching up multiple
-- tiers in one recompute (a big score jump) still only inserts one
-- notification for wherever they landed, not one per skipped tier.
...
-- Proactive counterpart to `get_profile_level`'s reactive staleness check
-- (same >1-day window) — lets a level-up (and its notification, above)
-- get detected even for a profile nobody's client happens to view that
-- day, which is what actually makes a push notification possible while
-- someone's away from the app. Not granted to `authenticated` — only the
-- cron job below calls it.
create or replace function public.refresh_stale_profile_levels()
returns void
language plpgsql
security definer
set search_path = public
...
begin
  for r in
    ...
    where id not in (select profile_id from public.profile_levels)
    union
    select profile_id from public.profile_levels
    where computed_at < now() - interval '1 day'
  loop
    perform public.refresh_profile_level(r.profile_id);
  end loop;
end;
$$;
...
select cron.schedule(
  'refresh-stale-profile-levels',
  '0 3 * * *',
  $$select public.refresh_stale_profile_levels();$$
);
