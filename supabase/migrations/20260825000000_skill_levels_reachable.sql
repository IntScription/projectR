-- Same issue as the overall level-100 gate (see
-- 20260824000000_level_100_reachable.sql), one level down: an individual
-- skill's score required 3 updates and 10 combined likes/comments *per
-- project that skill was used in* to max those two dimensions, and 3+
-- separate projects to max the last one — for a skill used in a single
-- project, that's structurally impossible to reach 100 on ("3 updates on
-- 1 project" maxes the ratio at 1/3, not 1/1). Only the `skill_scored`
-- CTE's three per-project-scaled dimensions change here; the 0.4/0.2/0.2/
-- 0.2 weights, the shipped-ratio dimension (already self-normalizing —
-- one shipped project already maxes it), and the uncapped post-100
-- prestige climb for skills are all unchanged.
--
-- New bar per skill: 2 updates total, 3 combined likes/comments total,
-- used in 2 projects — for a skill used in just one well-shipped,
-- engaged, updated project, that's 90/100 (only the "used in 2 projects"
-- dimension can't max out from a single project, which is a fair signal
-- to keep: real proficiency vs. one-off use).
  ...
  skill_scored as (
    select
      ...
      round(
        ...
        + least(updates_sum::numeric / 2, 1) * 100 * 0.2
        + least((likes_sum + comments_sum)::numeric / 3, 1) * 100 * 0.2
        + least(proj_uses::numeric / 2, 1) * 100 * 0.2
      ) as skill_score
    from skill_agg
  ),
  skill_leveled as (
